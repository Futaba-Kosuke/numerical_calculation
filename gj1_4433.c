...
// 上三角行列に整形する
int format_diagonal(int n, float A[6][7]);
// 上三角1次連立方程式の解を算出する
void calcute_diagonal(int n, float A[6][7], float solutions[], char solutions_char[]);
...
int main(void) {
  ...
  if(format_diagonal(n, A)) {
    calcute_diagonal(n, A, solutions, solutions_char);
  } else {
    printf("一意解が存在しません。\n");
  }
